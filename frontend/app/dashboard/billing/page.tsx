...
export default async function BillingPage({
  ...
  const plan = res.ok ? await res.json() : { plan: "free", gens_used: 0, gens_limit: 3, stripe_subscription_id: null, subscription_status: null };
    ...
    <div className="max-w-2xl">
      ...
      <div className="mb-6 rounded-xl border border-zinc-800 bg-zinc-900 p-5">
        ...
        {!isPro && (
          <div className="mb-5">
            ...
            <p className="mt-1 text-sm text-zinc-500">Unlimited content packs, multiple URLs per generation, and priority support.</p>
          </div>
        )}
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
        ...
        {hasSubscription ? (
          ...
          <p className="text-sm text-zinc-500">No payment method on file. Subscribe to a plan above to add one.</p>
        )}
      </div>
    </div>
  );
