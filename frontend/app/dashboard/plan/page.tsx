...
export default async function PlanPage() {
  ...
  const plan = res.ok ? await res.json() : { plan: "free", gens_used: 0, gens_limit: 3, stripe_subscription_id: null };
  ...
  const planSubtitle = isPro ? "Pro plan" : isBeginner ? "Beginner plan" : `Free trial — ${gensUsed} of ${gensLimit} generations used`;
        ...
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-2xl font-bold text-zinc-100">
              {planPrice}
              <span className="text-sm font-normal text-zinc-500">{isTrial ? " today" : " / month"}</span>
            </p>
          </div>
        </div>
      ...
      {!isPro && (
        <div className="rounded-xl border border-orange-500/20 bg-orange-950/20 p-6">
          ...
          <p className="mb-4 text-sm text-zinc-400">
            {isTrial ? "Choose a plan below to unlock more content packs." : "Unlimited content packs, multiple URL processing, and priority support."}
          </p>
